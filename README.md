...
- 📐 **Grammar patterns** — JLPT/CEFR patterns plus Genki 1 and JFZ grammar, taught as *patterns with audio examples*, explained in English, Japanese *and your native language*. Every pattern shows **at least ten example sentences** with a **Load 10 more** button to page through more (hand-written extras fill small gaps, and a short pattern may borrow from another pattern that teaches the **same** grammar point — never from an unrelated one, so a *past perfect continuous* sheet only ever shows *past perfect continuous* sentences). Every pattern also has a **🎯 Practice** button, and the Grammar tab has a dedicated **Practice** section behind a *Patterns / Practice* toggle: drills for every topic that needs one — tenses, articles, prepositions, modals, conditionals, reported speech, comparatives and more. Every one of the **47 topics carries 1000+ offline drills** (a hand-written core plus a generated bilingual template pack, `app/src/main/assets/practice_drills.txt`, rebuilt from `tools/practice-recipes.mjs`), and each session takes a **fresh random sample of ten**, so questions no longer repeat — a **+10 more** button extends the session. Crucially, all four answer options are the *same sentence* differing only in the grammar point being tested ("I am a boy" vs "I was a boy" vs "I is a boy"), so the drill can't be passed by guesswork the way a set of unrelated sentences can. **🎯 Practice random grammar** opens a **🎲 Random mix of every topic** card that draws from all topics at once (no more always-has/have or always-prepositions), and the vocab quiz now draws from the whole language with its own **10 more questions** button. See `docs/PRACTICE.md`.
...
- 🔟 **Real examples everywhere** — every vocabulary word's example sheet shows **real sentences that actually use that word**, drawn from the offline corpus, each with its own curated translation in your native language — never a shared sentence frame with the headword dropped into the blank, and never a machine-translated "meaning table" row. Examples are de-duplicated punctuation- and case-insensitively, so "Good night", "Good night!" and "Good night." never appear as three separate cards, and a row that is just the headword repeated is dropped. Words that are complete utterances — greetings and courtesy phrases like **good evening, how are you, thank you, see you later, I'm fine** — likewise show their natural real-life usage. The count on the sheet is honest: a word lists every example the bundle really has for it (usually several; common words ten or more), and a word with none simply says so rather than showing invented text.
- ➕ **Unlimited extra examples, in every language you can learn** — every word's real examples come from a bundled corpus, paged with **Load 10 more**: ~89,000 Tatoeba pairs (CC-BY 2.0 FR) for Japanese plus a per-language corpus for each of the other ten targets — a Tatoeba slice where one exists and ~29,000 generated sentences checked in at build time. Between them, **99–100 % of the taught words in every generated track have at least one bundled example** (2.5–4.3 per word on average). Every translation shown comes from a curated pair — a real Tatoeba row or a generated-and-checked sentence — so a card is never a word-by-word guess (Hindi alone carries 9,470 real Hindi↔English pairs). **Load 10 more** sits *below* the lines it adds, so the button never jumps off screen, and the same block is reachable from the word sheet, the lesson quiz, spaced-repetition reviews, the dictionary and the verb trainer, not just the word-sheet page — all offline, with no account, no key and no model download. A build-time script (`tools/build-ai-examples.mjs`) can top the corpus up further with a free key before you ship; see `docs/EXTRA-EXAMPLES.md` and `docs/DATA-LICENSES.md`.
...
The APK ships `arm64-v8a` + `armeabi-v7a` native code only (see the `ndk { abiFilters }` block in `app/build.gradle.kts`), which avoids shipping ~60 MB of emulator-only libraries.
