...
- 🔟 **Ten examples everywhere** — every vocabulary word and every grammar pattern shows exactly ten example sentences *in the language you're learning*, each with a gloss in your native language. `TargetExamples.kt` holds ten sentence frames per part of speech (noun/verb/adjective/adverb/number/other) in all 12 languages, and `Examples.exLines` blends any curated examples with those frames so no track ever falls back to English or shows only a handful.
