...
- ➕ **Unlimited extra examples, in every language you can learn** — every word's example sheet has a **More examples** button that streams real sentences from a bundled corpus: ~89,000 Tatoeba pairs (CC-BY 2.0 FR) for Japanese plus a per-language corpus for each of the other ten targets — a Tatoeba slice where one exists and ~23,000 AI-written sentences checked in at build time. Between them, **99–100 % of the taught words in every generated track have at least one bundled example** (2.5–4.3 per word on average). **Load 10 more** sits *below* the lines it adds, so the button never jumps off screen, and the same block — plus an optional **on-device AI** mode (Profile → Advanced) that invents brand-new sentences with a Qwen2.5 model running entirely on your phone, no account, no API key, no server — is reachable from the word sheet, the lesson quiz, spaced-repetition reviews, the dictionary, the verb trainer, not just the word-sheet page. A build-time script (`tools/build-ai-examples.mjs`) can top the corpus up further with a free key before you ship; see `docs/EXTRA-EXAMPLES.md` and `docs/DATA-LICENSES.md`.
